...
export default function About() {
  return (
    ...
    >
      ...
      <SectionDescription>
        Aqui estão algumas caracteristicas
        <br />
        minhas e ferramentas que uso
      </SectionDescription>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 md:grid-rows-[1fr_20px_1fr] gap-3.5 text-left">
        <CardAboutMe className="md:row-span-2 md:row-start-1 sm:col-span-2 md:col-start-1" />
        <CardBackend className="md:row-start-3" />
        <CardFrontend className="md:row-start-3 md:col-start-2" />
        <CardTechList className="sm:col-span-2 md:col-span-1 md:row-span-3 md:col-start-3" />
      </div>
    </section>
  );
}

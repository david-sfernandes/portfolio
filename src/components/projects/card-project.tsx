    ...
    <div className="card-layout card-project">
      ...
      <div className="tech-list">
        {project.tech_list.map((tech) => (
          <TechNode key={`${project.id}-${tech.toLowerCase()}`}>{tech}</TechNode>
        ))}
      </div>
      <div className="flex w-full gap-2.5">
        <button className="visit-btn" title="Visitar">Visitar <ArrowUpRightFromSquareIcon className="text-white/80 size-4.5 mt-0.5" /></button>
        <button className="repo-btn" title="Repositório GitHub">
          <Image src="/github.svg" width={24} height={24} className="size-6" alt="github"/>
        </button>
      </div>
    </div>
  )
